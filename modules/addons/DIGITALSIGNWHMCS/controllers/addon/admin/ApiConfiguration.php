...
class ApiConfiguration extends main\mgLibs\process\AbstractController
{
    ...
    public function indexHTML($input = [], $vars = [])
    {
        ...
        $field->value = $input['access_key_id'];
        $field->error = $this->getFieldError('access_key_id');
        $form->addField($field);

        $field        = new main\mgLibs\forms\TextField();
        $field->name  = 'access_key_secret';
        $field->value = $input['access_key_secret'];
        $field->error = $this->getFieldError('access_key_secret');
        $form->addField($field);

        $field        = new main\mgLibs\forms\SelectField();
        $field->name  = 'api_origin';
        $field->value = $input['api_origin'];
        $field->options = array(
            'https://api.digital-sign.com.cn' => '正式环境(Production ready)',
            'https://staging.api.digital-sign.com.cn' => '测试环境(Test only)',
            'https://dev.api.digital-sign.com.cn' => '开发环境(Inside only)'
        );
        $field->error = $this->getFieldError('api_origin');
        $form->addField($field);
        ...
        $field->continue          = false;
        $field->enableDescription = true;
        $field->options           = \MGModule\DIGITALSIGNWHMCS\eRepository\whmcs\config\Countries::getInstance()->getCountriesForMgAddonDropdown();
        ...
        $field->name              = 'send_certificate_template';
        $field->required          = true;
        $field->value             = ($input['send_certificate_template'] == NULL) ? \MGModule\DIGITALSIGNWHMCS\eServices\EmailTemplateService::getTemplate('SSLCenter - Send Certificate')->id : $input['send_certificate_template'];
        $field->translateOptions  = false;
        $field->inline            = true;
        $field->colWidth          = 4;
        $field->continue          = false;
        $field->enableDescription = true;
        $field->options           = $this->prepareGeneralEmailTemplatedArray(\MGModule\DIGITALSIGNWHMCS\eServices\EmailTemplateService::getGeneralTemplates());
        ...
        $vars['cronCommandLine']  = 'php -q ' . ROOTDIR . DS . 'modules' . DS . 'addons' . DS . 'DIGITALSIGNWHMCS' . DS . 'cron' . DS . 'cron.php';
        $vars['cronCommandLine2'] = 'php -q ' . ROOTDIR . DS . 'modules' . DS . 'addons' . DS . 'DIGITALSIGNWHMCS' . DS . 'cron' . DS . 'certificateStatsLoader.php';
        $vars['cronCommandLine3'] = 'php -q ' . ROOTDIR . DS . 'modules' . DS . 'addons' . DS . 'DIGITALSIGNWHMCS' . DS . 'cron' . DS . 'notifier.php';
        $vars['cronCommandLine4'] = 'php -q ' . ROOTDIR . DS . 'modules' . DS . 'addons' . DS . 'DIGITALSIGNWHMCS' . DS . 'cron' . DS . 'certificateSender.php';
        $vars['cronCommandLine5'] = 'php -q ' . ROOTDIR . DS . 'modules' . DS . 'addons' . DS . 'DIGITALSIGNWHMCS' . DS . 'cron' . DS . 'APIPriceUpdater.php';
        return array
            (
            //You have to create tpl file  /modules/addons/DIGITALSIGNWHMCS/templates/admin/pages/example1/example.1tpl
