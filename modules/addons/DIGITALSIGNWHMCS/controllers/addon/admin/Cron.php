...
namespace MGModule\DIGITALSIGNWHMCS\controllers\addon\admin;

use MGModule\DIGITALSIGNWHMCS\eServices\provisioning\ConfigOptions as C;
...
class Cron extends main\mgLibs\process\AbstractController
{
    ...
    public function indexCRON($input, $vars = array())
    {
        ...
        $this->sslRepo = new \MGModule\DIGITALSIGNWHMCS\eRepository\whmcs\service\SSL();
        ...
        foreach ($sslOrders as $sslService)
        {
            ...
            $order = \MGModule\DIGITALSIGNWHMCS\eProviders\ApiProvider::getInstance()->getApi()->getOrderStatus($sslService->remoteid);
        ...
        logActivity("DIGITALSIGN WHMCS: Synchronization completed. Number of synchronized services: " . count($updatedServices));
    ...
    public function notifyCRON($input, $vars = array())
    {
        ...
        foreach ($services as $srv)
        {
            ...
            if ($srv->billingcycle == 'One Time' && $srv->nextduedate == '0000-00-00')
            {
                ...
                $order    = \MGModule\DIGITALSIGNWHMCS\eProviders\ApiProvider::getInstance()->getApi()->getOrderStatus($sslOrder->remoteid);
        ...
        foreach($invoices as $invoice)
        {
            ...
            if(!empty($itemsInvoice))
            {
                ...
                $sslOrder    = \MGModule\DIGITALSIGNWHMCS\eProviders\ApiProvider::getInstance()->getApi()->getOrderStatus($sslInfo->remoteid);
        ...
        logActivity("DIGITALSIGN WHMCS: Notifier completed. Number of emails send: " . $emailSendsCount);
        logActivity("DIGITALSIGN WHMCS: Notifier completed. Number of invoiced created: " . $invoicesCreatedCount);
    ...
    public function certificateSendCRON($input, $vars = array())
    {
        ...
        logActivity("DIGITALSIGN WHMCS: Certificate Sender started.");
        ...
        foreach ($services->get() as $service)
        {
            ...
            {
                sendMessage(\MGModule\DIGITALSIGNWHMCS\eServices\EmailTemplateService::SEND_CERTIFICATE_TEMPLATE_ID, $service->id, [
        ...
        logActivity("DIGITALSIGN WHMCS: Certificate Sender completed. The number of messages sent: " . $emailSendsCount);
        return array();
    }
    ...
    public function loadCertificateStatsCRON($input, $vars = array())
    {
        ...
        logActivity("DIGITALSIGN WHMCS: Certificate Stats Loader started.");
        ...
        logActivity("DIGITALSIGN WHMCS: Certificate Stats Loader completed.");
        return array();
    }
    ...
    public function updateProductPricesCRON($input, $vars = array())
    {
        ...
        logActivity("DIGITALSIGN WHMCS: Products Price Updater started.");

        try
        {
            //get all products prices
            $apiProductsPrices = \MGModule\DIGITALSIGNWHMCS\eRepository\sslcenter\ProductsPrices::getInstance();
        ...
        {
            logActivity("DIGITALSIGN WHMCS Products Price Updater Error: " . $e->getMessage());
        }

        echo '<br/ >';
        echo 'Products Price Updater completed.' . PHP_EOL;
        logActivity("DIGITALSIGN WHMCS: Products Price Updater completed.");
        return array();
    }
    ...
    public function sendExpireNotfiyEmail($serviceId, $daysLeft)
    {
        ...
        if (!$resultSuccess)
        {
            logActivity('DIGITALSIGN WHMCS Notifier: Error while sending customer notifications (service ' . $serviceId . '): ' . $results['message'], 0);
        }
