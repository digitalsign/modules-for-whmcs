...
class Migration
{
    ...
    public function run()
    {
        try
        {
            logActivity('DIGITALSIGN WHMCS: Data & Configuration Migration Started');
            ...
            logActivity('DIGITALSIGN WHMCS: Data & Configuration Migration Completed');
        }
        catch (Exception $ex)
        {
    ...
    private function importModuleConfiguration()
    {
        if (self::$moduleLoaded)
        {
            ...
            logActivity('DIGITALSIGN WHMCS: Module '.self::MODULE_NAME.' configuration imported successfully.');
        }
        else
        {
            logActivity('DIGITALSIGN WHMCS: Module '.self::MODULE_NAME.' installation not detected.');
        }
    }
    ...
    private function updateExistingProducts()
    {
        ...
        foreach ($productsRepo->get() as $product)
        {
            ...
            logActivity('DIGITALSIGN WHMCS: Product ID: ' . $product->id . ' has been modified. Module changed from ' . self::MODULE_NAME . ' to ' . main\Addon::I()->configuration()->systemName);
        }
    }
    ...
    private function updateSSLOrders()
    {
        ...
        foreach ($orders as $ssl)
        {
            ...
            logActivity('DIGITALSIGN WHMCS: SSL Service ID: ' . $ssl->serviceid . ' has been modified. Module changed from ' . self::MODULE_NAME . ' to ' . main\Addon::I()->configuration()->systemName);
