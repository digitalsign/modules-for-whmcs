...
use \DateInterval;
use \DateTime;
use \MGModule\DIGITALSIGNWHMCS\mgLibs\MySQL\Query;
use \MGModule\DIGITALSIGNWHMCS\eServices\provisioning\ConfigOptions;
...
class Invoice
{
    ...
    {
        $clientRepo = new \MGModule\DIGITALSIGNWHMCS\models\whmcs\clients\Client($id);
        return $clientRepo->getCurrencyId();
    }
    
    protected function getProductPricing($id)
    {
        $productRepo = new \MGModule\DIGITALSIGNWHMCS\models\productConfiguration\Repository();     
    ...
    protected function getProperServiceBillingCycleBasedOnProductPricing($service)
    {
        ...
        { 
            if($pricing->currency == $clientCurrencyID)
            {
                ...
                { 
                    if(key_exists($key, \MGModule\DIGITALSIGNWHMCS\models\whmcs\pricing\BillingCycle::PERIODS) && $priceFieldValue != '-1.00')
                    {                        
                        $period = \MGModule\DIGITALSIGNWHMCS\models\whmcs\pricing\BillingCycle::convertStringToPeriod($key);
        ...
        return [
            'key' => \MGModule\DIGITALSIGNWHMCS\models\whmcs\pricing\BillingCycle::convertPeriodToName($highestAvaialblePeriod) ,
    ...
    public function createInvoice($service, $product, $returnInvoiceID = false) {
        ...
        $commission = \MGModule\DIGITALSIGNWHMCS\eHelpers\Commission::getCommissionValue(array('pid' => $service->packageid, 'client' => $service->userid));
        ...
        {   
            ...
            {
                $productConfiguration = Capsule::table("tblproducts")->where("tblproducts.servertype", "=", "DIGITALSIGNWHMCS")->where("id", "=", $service->packageid)->first();
                $timeShift = 'P' . $productConfiguration->{ConfigOptions::API_PRODUCT_MONTHS} . 'M';            
            }
            else
                $timeShift = 'P' . \MGModule\DIGITALSIGNWHMCS\models\whmcs\pricing\BillingCycle::convertStringToPeriod($availableBillingCycle['key']) . 'M';
    ...
    public function CopyConfigurationAndSendRenewToGGSSL($newOrderId)
    {
        ...
        $sslOrder = \MGModule\DIGITALSIGNWHMCS\eProviders\ApiProvider::getInstance()->getApi()->getOrderStatus($sslOrderInfo->remoteid);
        ...
        $addedSSLOrder = \MGModule\DIGITALSIGNWHMCS\eProviders\ApiProvider::getInstance()->getApi()->addSSLRenewOrder($order);
    ...
    private function getSanCountConfigOptionServiceDetails($service)
    {
        ...
        {            
            ...
            {
                logActivity("DIGITALSIGN WHMCS: Required Product Module '" . $server->getServiceModule() . "' Missing"); 
            }
            else
            {
                $serviceParams = $server->buildParams(); 
                if (!empty($serviceParams)) {         
                    $CORepo = new \MGModule\DIGITALSIGNWHMCS\models\whmcs\service\configOptions\Repository($service->id);
    ...
    {        
        try
            {
            $service = new \MGModule\DIGITALSIGNWHMCS\models\whmcs\service\Service($serviceID);       
            ...
            $invoiceItemsRepo = new \MGModule\DIGITALSIGNWHMCS\models\whmcs\invoices\RepositoryItem();        
            ...
            {     
                ...
                if($newDescription)
                {
                    ...
                    logActivity("DIGITALSIGN WHMCS: Description of the invoice item for Invoice ID: " . $invoice->getId() . ' has been changed from "' . $oldDescription . '" to "' . $newDescription . '"');
                }
            } 
        }
        catch(\Exception $e)
