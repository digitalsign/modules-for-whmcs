...
class ClientReissueCertificate {
    ...
    private function miniControler() {
        ...
        if (isset($this->post['stepOneForm'])) {           
            try {
                ...
                $this->vars['errors'][] = \MGModule\DIGITALSIGNWHMCS\eHelpers\Exception::e($ex);
            }
        }
        ...
        $this->vars['generateCsrIntegrationCode'] =   ($displayCsrGenerator) ? \MGModule\DIGITALSIGNWHMCS\eServices\ScriptService::getGenerateCsrModalScript(json_encode(array()), $countriesForGenerateCsrForm) : '';       
    ...
    private function stepOneForm() {
        ...
        $decodeCSR = \MGModule\DIGITALSIGNWHMCS\eProviders\ApiProvider::getInstance()->getApi(false)->decodeCSR($this->post['csr']);
        if ($decodeCSR['error']) {
            if(isset($decodeCSR['description']))
                throw new Exception($decodeCSR['description']);
            
            throw new Exception(\MGModule\DIGITALSIGNWHMCS\mgLibs\Lang::getInstance()->T('incorrectCSR'));
        }
        
        $mainDomain                   = $decodeCSR['csrResult']['CN'];
        $domains                      = $mainDomain . PHP_EOL . $this->post['sans_domains'];
        $parseDomains                 = \MGModule\DIGITALSIGNWHMCS\eHelpers\SansDomains::parseDomains(strtolower($domains));
    ...
    private function stepTwoForm() {
        ...
        $decodedCSR   = \MGModule\DIGITALSIGNWHMCS\eProviders\ApiProvider::getInstance()->getApi(false)->decodeCSR($this->post['csr']);
        if ($this->getSansLimit() AND count($_POST['approveremails'])) {
            $this->validateSanDomains();
            $sansDomains             = \MGModule\DIGITALSIGNWHMCS\eHelpers\SansDomains::parseDomains($this->post['sans_domains']);
        ...
        $orderStatus = \MGModule\DIGITALSIGNWHMCS\eProviders\ApiProvider::getInstance()->getApi()->getOrderStatus($this->sslService->remoteid);        
        ...
        if (count($sansDomains) > $orderStatus['total_domains'] AND $orderStatus['total_domains'] >= 0) {
            ...
            \MGModule\DIGITALSIGNWHMCS\eProviders\ApiProvider::getInstance()->getApi()->addSslSan($this->sslService->remoteid, $count);
        }
        
        \MGModule\DIGITALSIGNWHMCS\eProviders\ApiProvider::getInstance()->getApi()->reIssueOrder($this->sslService->remoteid, $data);
    ...
    private function validateWebServer() {
        if($this->post['webservertype'] == 0) {
            throw new Exception(\MGModule\DIGITALSIGNWHMCS\mgLibs\Lang::getInstance()->T('mustSelectServer'));
        }
    }
    ...
    private function validateSanDomains() {
        ...
        if (count($invalidDomains)) {
            throw new Exception(\MGModule\DIGITALSIGNWHMCS\mgLibs\Lang::getInstance()->T('incorrectSans') . implode(', ', $invalidDomains));
        }
        ...
        if (count($sansDomains) > $sansLimit) {
            throw new Exception(\MGModule\DIGITALSIGNWHMCS\mgLibs\Lang::getInstance()->T('exceededLimitOfSans'));
        }
    ...
    private function validateService() {
        ...
        if (is_null($this->sslService)) {
            throw new Exception(\MGModule\DIGITALSIGNWHMCS\mgLibs\Lang::getInstance()->T('createNotInitialized'));
        }

        $this->orderStatus = \MGModule\DIGITALSIGNWHMCS\eProviders\ApiProvider::getInstance()->getApi()->getOrderStatus($this->sslService->remoteid);

        if ($this->orderStatus['status'] !== 'active') {
            throw new Exception(\MGModule\DIGITALSIGNWHMCS\mgLibs\Lang::getInstance()->T('notAllowToReissue'));
        }
    ...
    private function loadServerList() {
        ...
        try {
            $apiRepo                  = new \MGModule\DIGITALSIGNWHMCS\eRepository\sslcenter\Products();
            $apiProduct               = $apiRepo->getProduct($this->p[\MGModule\DIGITALSIGNWHMCS\eServices\provisioning\ConfigOptions::API_PRODUCT_ID]);
            ...
            \MGModule\DIGITALSIGNWHMCS\eServices\FlashService::set('DIGITALSIGNWHMCS_SERVER_LIST_' . \MGModule\DIGITALSIGNWHMCS\eServices\provisioning\ConfigOptions::API_PRODUCT_ID, $apiWebServers);
        } catch (Exception $ex) {
            $this->vars['errors'][] .= \MGModule\DIGITALSIGNWHMCS\mgLibs\Lang::getInstance()->T('canNotFetchWebServer');
        }
    ...
    private function build($template) {
        ...
        $content             = \MGModule\DIGITALSIGNWHMCS\eServices\TemplateService::buildTemplate($template, $this->vars);
        return [
            'templatefile' => 'main',
