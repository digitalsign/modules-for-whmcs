...
class ConfigOptions
{
    ...
    {
        $apiProducts = \MGModule\DIGITALSIGNWHMCS\eRepository\sslcenter\Products::getInstance()->getAllProducts();
        $products    = [];

        foreach ($apiProducts as $apiProduct)
        {
            $products[$apiProduct->id] = $apiProduct->name;
        }
    ...
    private function getErrorOptions($error)
    {
        return [
            'An Error Occurred:' => [
                'Type'        => 'text',
                'Description' => \MGModule\DIGITALSIGNWHMCS\eServices\ScriptService::getConfigOptionErrorScript($error)
            ]
        ];
    }
